...
begin
  Result := TControlAccess(FForm).IsControl;
end;
...
begin
  Result := (FForm.FindComponent(AName) = nil);
end;
...
begin
  TControlAccess(FForm).IsControl := Value;
end;
  ...
  begin
    try
      ...
      try
        ...
        if GetCaptureControl = FForm then
        ...
        begin
          ...
          begin
            ...
            try
              ...
              begin
                if not ((Sender = FForm) and (FSelCtrls.Count <> 0)) then
...
type
  ...
  function _FindNearControl(AForward: Boolean): TControl;
    ...
    begin
      ...
      begin
        if (FForm.Components[AFromIndex] is TControl) and
          (TControl(FForm.Components[AFromIndex]).Parent <> nil) then
        begin
          Result := TControl(FForm.Components[AFromIndex]);
          Break;
        end;
    ...
    begin
      ...
      begin
        Result := __DoFind(LMainControl.ComponentIndex +
          (-1 + 2 * Ord(AForward)), (FForm.ComponentCount - 1) *
          Ord(AForward), AForward);
        if Result = nil then
          Result := __DoFind((FForm.ComponentCount - 1) * Ord(not AForward),
        ...
        case TWMKeyDown(Message).CharCode of
          ...
          VK_LEFT, VK_UP, VK_RIGHT, VK_DOWN:
            ...
            begin
              ...
              begin
                if FSelCtrls.DefaultControl <> FForm then
                begin
                  FSelCtrls.BeginUpdate;
                  try
...
begin
  if ((FSelCtrls.Count = 1) and (FSelCtrls[0] <> FForm)) or
  ...
  case AFlag of
    ...
    pfOkRemoveAndDispose:
      try
        ...
        if PDataRec(AOldDataPtr)^.Drawn then
          _DoXORDraw(LDrawRect);
        LInsertingControl := FInsertedControlClass.Create(FForm);
        try
...
begin
  Result := IsWindowVisible(FForm.Handle);
end;
...
begin
  if (AControl <> FForm) and IsDesignControl(AControl) then
  begin
...
begin
  Result := (AControl = FForm) or
    ((csDesigning in AControl.ComponentState) and (AControl.Owner = FForm));
end;
...
var
  LIntLockMode: Integer;
begin
  if IsDesignControl(AControl) and (AControl <> FForm) then
  begin
  ...
  try
    ...
    try
      ...
      try
        ...
        LR.ReadComponents(FForm, AParent, ReaderReadComponent);
      finally
        LR.Free;
      end;
    finally
...
begin
  if Active and
    ...
    (SelectedControls[0] <> TDEng(FEng).FForm)) or
  ...
  begin
    ...
    for LI := 0 to SelectedControls.Count - 1 do
      if
        ...
        and (SelectedControls[LI] <> TDEng(FEng).FForm)
...
begin
  CheckActive(True);
  for LI := 0 to TDEng(FEng).FForm.ComponentCount - 1 do
    if TDEng(FEng).FForm.Components[LI] is TControl then
      TDEng(FEng).LockControl(TControl(TDEng(FEng).FForm.Components[LI]),
        ALockMode);
end;
...
begin
  FDesigner.CheckActive(True);
  for LI := 0 to Count - 1 do
    if Items[LI] <> TDEng(FDesigner.FEng).FForm then
...
begin
  ...
  if (DefaultControl <> nil) and
    (DefaultControl <> TDEng(FDesigner.FEng).FForm) then
  begin
