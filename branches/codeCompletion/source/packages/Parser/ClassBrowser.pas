...
type
  ...
  private
    ...
    procedure AddMembers(Node: TTreeNode; ParentIndex, ParentID: Integer; const ClassIDs: TIntList);
...
procedure TClassBrowser.AddMembers(Node: TTreeNode; ParentIndex, ParentID: Integer; const ClassIDs: TIntList);
var
...
begin
  ...
  {if ParentIndex <> -1 then with PStatement(fParser.Statements[ParentIndex])^ do
  begin
  ...
  else if HasSubFolder('') then
    CreateFolders('', Node);}

  sl := TStringList.Create;
  try
    // allow inheritance propagation
    {if fShowInheritedMembers and (ParentIndex <> -1) then
    begin
...
begin
  ...
  ClassIDs := fParser.FilterByType([itClass]);
  AddMembers(nil, -1, -1, ClassIDs);
  ClassIDs.Free;
...
begin
  ...
  if High(fFolders) = -1 then begin
    DeleteFile(PChar(fProjectDir + '\' + fClassFoldersFile));
    Exit;
  end;
