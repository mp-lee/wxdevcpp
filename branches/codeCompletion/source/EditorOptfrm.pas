...
{$IFDEF WIN32}
  ...
  Buttons, ClassBrowser, CppParser, XPMenu, Classes, StrUtils;
{$ENDIF}
{$IFDEF LINUX}
...
type
  TEditorOptForm = class(TForm)
    ...
    procedure CppParser1FileProgress(Sender: TObject; FileName: String;
...
var
  ...
  flt: string;
begin
  //TODO: lowjoel: Insert the compiler include paths in herer
  try
...
//    sl.Free;
  ...
  with dmMain do begin
    ...
    begin
      EditorCCForm := TEditorCCForm.Create(Self);
      Screen.Cursor := crHourglass;
      ...
      CppParser1.OnFileProgress := CppParser1FileProgress;
      CppParser1.RecursionDepth := 1024;

      //Add the files to scan
      for I := 0 to OpenDialog.Files.Count - 1 do
        CppParser1.QueueFile(OpenDialog.Files[I]);
      chkCCCache.Tag := 1; // mark modified

      //Show the progress dialog
      EditorCCForm.ShowModal;
      EditorCCForm.Free;
...
begin
  if lbCCC.Items.Count = 0 then
    Exit;
  if MessageDlg('Are you sure you want to clear the Code Completion Cache?',
...
begin
  ...
  Files := CppParser1.ListFiles;
  lbCCC.Items.BeginUpdate;
  for I := 0 to Files.Count - 1 do
    lbCCC.Items.Add(CompactFilename(Files[I]));
  Files.Free;
...
begin
  EditorCCForm.btnOK.Enabled := True;
  EditorCCForm.lblStatus.Caption := 'Complete.';
  EditorCCForm.pbOverall.Position := EditorCCForm.pbOverall.Max;
  EditorCCForm.pbCurrent.Position := EditorCCForm.pbCurrent.Max;
  EditorCCForm.Close;
end;

procedure TEditorOptForm.CppParser1FileProgress(Sender: TObject; FileName: String;
      Total, Current: Integer);
begin
  EditorCCForm.lblCurrent.Caption := 'Parsing: ' + FileName;
  EditorCCForm.pbCurrent.Max := Total;
  EditorCCForm.pbCurrent.Position := Current;
end;

procedure TEditorOptForm.CppParser1TotalProgress(Sender: TObject;
  FileName: String; Total, Current: Integer);
begin
  EditorCCForm.lblCurrent.Caption := 'Preprocessing: ' + FileName;
  EditorCCForm.pbOverall.Max := Total;
  EditorCCForm.pbOverall.Position := Current - 1;
  EditorCCForm.pbCurrent.Position := 0;
end;
