...
{$IFDEF WIN32}
  ...
  debugger, ClassBrowser, DevThemes, CodeCompletion, CppParser,
  JvComponentBase, JvDockControlForm, XPMenu, DdeMan, devShortcuts,
  StrUtils, devFileMonitor, devMonitorTypes, XPToolTip, CodeToolTip,
  CVSFm, ImageTheme
...
{$ENDIF}
  ...
  CppParser1.LoadCache(devDirs.Config + DEV_COMPLETION_CACHE);
  ...
  for I := 0 to Callstack.Count - 1 do
    ...
    begin
      ...
      if PStackFrame(Callstack[I])^.Line <> 0 then
        ...
        //TODO: lowjoel: Code Completion-assisted stack tracing?
...
{$ENDIF}
      ...
      begin
        CppParser1.QueueFile(e.FileName);
...
{$ENDIF}
      ...
      try
        ...
        begin
          CppParser1.QueueFile(fProject.units[idx].FileName);
          if e.TabSheet = PageControl.ActivePage then
            //ClassBrowser1.CurrentFile := fProject.units[idx].FileName;
        end;
        Result := True;
      except
        MessageDlg(Format('Error parsing source file %s', [e.FileName]), mtError, [mbOk], Handle);
      end;
    end
...
{$IFDEF WX_BUILD}
    ...
    begin
        ...
        begin
            if ClassBrowser1.Enabled then
                CppParser1.QueueFile(hFile);
        end;

        if (bCppModified =true ) then
        begin
            if ClassBrowser1.Enabled then
                CppParser1.QueueFile(cppFile);
        end;
    end
    else
   {$ENDIF}
...
begin
  ...
  if not Assigned(fProject) and (PageControl.PageCount = 0) then begin
    //TODO: lowjoel: re-load the pre-compiled list of identifiers
    CppParser1.LoadCache(devDirs.Config + DEV_COMPLETION_CACHE);
//    ClassBrowser1.Clear;
  end;
end;
...
//  ClassBrowser1.ProjectDir := '';
  //TODO: lowjoel: re-load the compiled list of identifiers
  CppParser1.LoadCache(devDirs.Config + DEV_COMPLETION_CACHE);
  RefreshTodoList;
end;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        //TODO: lowjoel: The CppParser could have had a few settings changed. So we
        //               may have to dispose of the current cache, re-load from disk
        //               and re-parse the project.
        CppParser1.LoadCache(devDirs.Config + DEV_COMPLETION_CACHE);
        {ScanActiveProject;}
      end;
    ...
    begin
      ...
      try
        ...
        begin
          ...
          CppParser1.QueueFile(Files[idx]);
        end;
      finally
...
//TODO: lowjoel: Analyze all calls to InitClassBrowser and see what's the best
//               implementation
...
//TODO: lowjoel: The Class Browser code here doesn't make much sense.
    CppParser1.Reset;
    ClassBrowser1.Clear;
  end;
  ...
  if Full then begin
    ...
    with CppParser1 do begin
      ...
      //Add the include paths to the parser
      sl := TStringList.Create;
      try
        ExtractStrings([';'], [' '], PChar(devDirs.C), sl);
...
//TODO: lowjoel: this function needs to have a looksie
...
//      ProjectDir := fProject.Directory;
      for I := 0 to fProject.CurrentProfile.Includes.Count - 1 do
        AddIncludePath(fProject.CurrentProfile.Includes[I]);
      for I := 0 to fProject.Units.Count - 1 do
        QueueFile(fProject.Units[I].FileName);
    end
    else begin
      e := GetEditor;
      if Assigned(e) then begin
//        ProjectDir := ExtractFilePath(e.FileName);
        QueueFile(e.FileName);
      end
      else
//        ProjectDir := '';
    end;
...
begin
  with TFunctionSearchForm.Create(Self) do begin
    ...
    fParser := CppParser1;
    if ShowModal = mrOK then begin
      //TODO: lowjoel: Get this to work again
...
//TODO: lowjoel: The following events deal with the Class Browser. Fix it to work
//               when the class browser is rewritten
...
begin
  ...
  begin
    LineNo := StrToIntDef(lvBacktrace.Selected.SubItems[2], -1);
    if LineNo <> -1 then
    begin
      //TODO: lowjoel: code Completion-assisted stack tracing?
      E := GetEditorFromFileName(lvBacktrace.Selected.SubItems[1]);
      if Assigned(e) then
      begin
...
begin
  for I := 0 to lvBacktrace.Items.Count - 1 do
    ...
    begin
      LineNo := StrToIntDef(lvBacktrace.Items[I].SubItems[2], -1);
      if LineNo <> -1 then
      begin
        //TODO: lowjoel: code Completion-assisted stack tracing?
        E := GetEditorFromFileName(lvBacktrace.Items[I].SubItems[1]);
        if Assigned(E) then
        begin
          E.SetActiveBreakpointFocus(LineNo);
          Break;
        end;
      end;
    end;
...
var
  ...
  CurrentClass: TParseIdentifier;
  Classes: TIntList;
begin
  //Store the currently selected class
  SelectedClass := cmbClasses.Text;
  cmbClasses.Clear;
  cmbMembers.Clear;
  ...
  Classes := CppParser1.FilterByType([itClass]);
  
  for I := 0 to Classes.Count - 1 do begin
    CurrentClass := CppParser1.GetIdentifierDetails(Classes[I]);
    cmbClasses.Items.AddObject(CurrentClass.Name, TObject(CurrentClass.ID));
    CurrentClass.Free;
  end;
  Classes.Free;

  //Finished updating class list
  cmbClasses.Items.EndUpdate;

  //Re-select the stored class
  if SelectedClass <> '' then begin
    cmbClasses.ItemIndex := cmbClasses.Items.IndexOf(SelectedClass);
    cmbClasses.OnChange(cmbClasses);
  end;
end;
...
var
  I: Integer;
  ClassID: Int64;
  Descendants: TIntList;
  CurrentDescendant: TParseIdentifier;
begin
  if cmbClasses.ItemIndex = -1 then
    Exit;

  ClassID := Int64(cmbClasses.Items.Objects[cmbClasses.ItemIndex]);
  Descendants := CppParser1.GetDescendantsOf(ClassID);
  cmbMembers.Items.BeginUpdate;
  cmbMembers.Clear;

  for I := 0 to Descendants.Count - 1 do
  begin
    CurrentDescendant := CppParser1.GetIdentifierDetails(Descendants[I]);
    if Assigned(CurrentDescendant) then
    begin
      cmbMembers.Items.AddObject(CurrentDescendant.Name, TObject(CurrentDescendant.ID));
      CurrentDescendant.Free;
    end;
  end;
...
begin
  ...
  FunctionID := Int64(cmbMembers.Items.Objects[cmbMembers.ItemIndex]);
  Identifier := CppParser1.GetIdentifierDetails(FunctionID);
  E := GetEditorFromFilename(Identifier.DeclarationFile);
  if Assigned(E) then
    E.GotoLineNr(Identifier.DeclarationLine);
  Identifier.Free;
end;
...
//TODO: EVERYONE: Refactorise this function. The code is becoming spaghetti!
...
//TODO: EVERYONE: This is a Designer helper function. Can we use the new Code Completion
//                design paradigm to get all the functions instead?
...
//TODO: EVERYONE: We need to document this function. What does it do?
